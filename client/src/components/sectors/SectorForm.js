...
const SectorForm = ({ current, validateSymbolIEX, validateSector, addOrUpdateSector, history }) => {
    ...
    const validateFields = async (field) => {
        ...
        switch (field) {
            ...
            case 'symbol':
                ...
                } else if (current === null || symbol !== current.symbol) {
    ...
    return (
        <div>
            <h3 className='text-center'>{current === null ? 'Add Sector' : 'Edit Sector'}</h3>
            <div className='col-md-8 m-auto pt-3'>
                <div className='card card-body border-secondary card-shadow pb-1'>
